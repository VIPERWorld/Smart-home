...
__packed	typedef struct
{
    Pro_HeadPartTypeDef  					Pro_HeadPart;//模组头
    uint8_t									Pro_ver[8];//模组版本
    uint8_t									P0_ver[8];//报文
    uint8_t									Hard_ver[8];//硬件版本
    uint8_t									Soft_ver[8];//软件版本
    uint8_t									Product_Key[32];//产品序列号
...
#define 		Wifi_ConfigMode       		  	(uint8_t)(1<<2)    //配置
#define 		Wifi_BindingMode     			(uint8_t)(1<<3)    //捆绑
#define     	Wifi_ConnRouter             	(uint8_t)(1<<4)    //控制路由器
#define     	Wifi_ConnClouds             	(uint8_t)(1<<5)    //控制机智云
/*****************************************************
