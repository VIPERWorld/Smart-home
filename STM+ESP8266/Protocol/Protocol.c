...
/*******************************************************************************
* Function Name  : exchangeBytes，大端，小端模式转换
...
void GizWits_init(uint8_t P0_Len)
{
    ...
    memset((uint8_t *)&g_DevStatus, 0, MAX_P0_LEN);//将数组g_DevStatus初始化为全0
    //memset(&Pro_M2W_ReturnInfoStruct, 0, sizeof(Pro_M2W_ReturnInfoStruct));//wifi信息的结构体初始化

    //Pro_M2W_ReturnInfoStruct.Pro_HeadPart.Head[0] = 0xFF;
    //Pro_M2W_ReturnInfoStruct.Pro_HeadPart.Head[1] = 0xFF;
    //Pro_M2W_ReturnInfoStruct.Pro_HeadPart.Len = exchangeBytes(sizeof(Pro_M2W_ReturnInfoStruct) - 4);//对数组的大小的值做大小端转换
    //Pro_M2W_ReturnInfoStruct.Pro_HeadPart.Cmd = Pro_D2W__GetDeviceInfo_Ack_Cmd;//获取wifi设备信息应答指令
    //memcpy(Pro_M2W_ReturnInfoStruct.Pro_ver, PRO_VER, strlen(PRO_VER));//把PRO_VER拷贝到结构体中
...
/*void GizWits_D2WConfigCmd(uint8_t WiFi_Mode)
{
    ...
    Pro_D2WConfigWiFiMode.Sum = CheckSum((uint8_t *)&Pro_D2WConfigWiFiMode, sizeof(Pro_D2W_ConfigWifiTypeDef));//对结构体进行和校验
